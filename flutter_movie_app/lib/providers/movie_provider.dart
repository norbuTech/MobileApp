...
class MovieProvider extends ChangeNotifier {
  ...
  List<MovieModel> get trending => _movies.sublist(0, 5);

  List<MovieModel> get latestMovies => _movies.sublist(_movies.length - 5);
...
//MovieCategory;
  Future<void> loadMoviesCategory() async{
    final res = await http.get(Uri.parse('https://yts.mx/api/v2/list_movies.json?genre={category}'));

    // decode data before use
    print(jsonDecode(res.body)['data']['movies']);

  List<MovieModel> moviesCategory=[];

    // looping for it to fit in MovieModel format
    for(final movieCategory in jsonDecode(res.body)['data']['movies']){
      moviesCategory.add(MovieModel.fromJson(movieCategory));
    }

    _movies= moviesCategory;
    notifyListeners();
  }
}
